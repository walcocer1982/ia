...
import type { Lesson, Moment, Step, AskStep } from '../domain/lesson.js';
import type { LessonSession, InteractionLog } from '../domain/session.js';
import type { SophiaResponse } from '../domain/responses.js';
import type { StructuredAgentMessage } from '../domain/agent-messages.js';
...
import type { Role, RoleDecision } from '../agents/educational-roles.js';
...
export interface ProcessStepResult {
  ...
  instructions: string;
  structuredMessages: StructuredAgentMessage[];
  momentCode: string;
  stepCode: string;
}
...
export class SophiaMultiAgentService {
  ...
  async processUserInput(session: LessonSession, userInput: string): Promise<ProcessStepResult> {
...
function buildEvaluationGuidelines(objective: string | undefined, attempts: number): string {
  const lines: string[] = [];
...
function describeHintStage(attempts: number): string {
  if (attempts <= 1) {
...
function buildFallbackQuestion(objective: string | undefined, attempts: number): string | undefined {
...
function logEvaluationOutcome(momentCode: string, stepCode: string, attempts: number, response: SophiaResponse): void {
  const summary = `[Evaluador] ${momentCode}-${stepCode} intento ${attempts}: ${response.classification} | accion ${response.nextAction}`;
  console.info(summary);
}

function buildPrimaryMessage(role: Role, response: SophiaResponse): StructuredAgentMessage {
  return {
    role,
...
function mapClassificationToMessageType(classification: SophiaResponse['classification'], hasQuestion: boolean) {
