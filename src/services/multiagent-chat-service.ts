...
import { SophiaMultiAgentService, type ProcessStepResult } from './sophia-multiagent-service.js';
...
export class MultiAgentChatService {
  constructor(private readonly sophiaService = new SophiaMultiAgentService()) {}

  createSession(lesson: Lesson, sessionId: string): ChatSession {
    const lessonSession = this.sophiaService.createSession(lesson, sessionId);
    return createChatSession(lessonSession);
  }

  async processUserInput(chatSession: ChatSession, userInput: string): Promise<ProcessChatResult> {
    ...
    const nextChatSession: ChatSession = {
