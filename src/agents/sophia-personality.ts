import type { EvaluationClassification } from '../domain/responses.js';
import type { AnswerType, Lesson, Moment } from '../domain/lesson.js';
...
export interface PersonalityContext {
  ...
  lastClassification?: EvaluationClassification;
  lastAction?: string;
}

const momentIntentMap: Record<string, { purpose: string; focus: string; interaction: string }> = {
  M1: {
...
export class SophiaPersonality {
  ...
  static buildContext(context: PersonalityContext): string {
  ...
  static buildToneGuidelines(): string {
    return [
  ...
  static buildInstructions(context: PersonalityContext): string {
    return [
      this.buildBaseVoice(),
      '',
