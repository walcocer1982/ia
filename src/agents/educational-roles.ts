import type { PersonalityContext } from './sophia-personality.js';
...
export class EducationalRoles {
  static getInstructions(role: Role, context: PersonalityContext): string {
    ...
    const sections: string[] = [baseHeader, intro, reminders, firstMomentReminder];
  ...
  static decideRoleFromInput(userInput: string, context: PersonalityContext): RoleDecision {
    ...
    const expressesClarification =
      clarificationPhrases.some(phrase => normalized.includes(phrase)) ||
